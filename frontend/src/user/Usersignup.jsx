import React,{useState} from 'react'

const Usersignup = () => {
      const [email,setEmail]=useState("")
      const [firstname,setFirstname]=useState("")
      const [lastname,setLastname]=useState("")
      const [phone,setPhone]=useState("");
      const [password,setPassword]=useState("")
       const [error,setError]=useState(false);
          const [data,setData]=useState(false);
       const [disable,setDisable]=useState(false)
      const handleSubmit=async(e)=>{
        e.preventDefault();
        setError(false);
        setData(false)
        const response=await fetch('http://localhost:4000/api/user/signup',{
          method:'post',
          headers:{
            'Content-Type':'application/json'
          },
          body:JSON.stringify({
            firstname,
            lastname,
            phone,
            email,
            password
          })
        })
        
        
        if(!response.ok){
          console.log('error');
          
          setError(true);
          setData(false)
          
        }else{
          setError(false)
          const val=await response.json();
          console.log(val);
          
          console.log(val);
          
          setData(true)
          
        }
      
      }

  return (
    <div className='signup w-[400px] h-[600px] rounded-lg bg-gradient-to-r from-gray-900 to-gray-600    text-green-500'>
      
    <form action="" onSubmit={(e)=>{
      setDisable(true);
      handleSubmit(e);
      
      }} className='w-full h-full flex flex-col justify-center items-center gap-3'>
      <input type="text" name='firstname' className=' w-[90%] h-[50px] pl-2 rounded-lg    ' placeholder='First Name' value={firstname} onChange={(e)=>setFirstname(e.target.value)} />
      <input type="text" name='lastname' className=' w-[90%] h-[50px] pl-2 rounded-lg     ' placeholder='Last Name' value={lastname} onChange={(e)=>setLastname(e.target.value)} />
      <input type="text" name='phone' className=' w-[90%] h-[50px] pl-2 rounded-lg     ' value={phone} onChange={(e)=>setPhone(e.target.value)}  placeholder='Phone ' />
      <input type="email" name='email' className='w-[90%] h-[50px] pl-2 rounded-lg    ' value={email} onChange={(e)=>setEmail(e.target.value)} placeholder='Email' />
      <input type="password" name='password' className=' w-[90%] h-[50px] pl-2 rounded-lg    ' value={password} onChange={(e)=>setPassword(e.target.value)} placeholder='Password' />
      <input type="submit" disabled={disable}  className='w-[100px] py-2 px-3 rounded-lg  text-black  bg-green-500 hover:bg-white' />
    </form>
    {error&&<div className='text-red-600'>something went wrong</div>}
    {data&&<div className='text-green-600'>logged in successfully</div>}
    </div>
  )
}

export default Usersignup
